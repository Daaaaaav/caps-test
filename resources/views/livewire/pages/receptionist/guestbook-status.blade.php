    ...
    <main class="px-4 sm:px-6 py-6 space-y-6">
        ...
        <div class="flex flex-wrap gap-3">
            <div class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#4E653D]/10 border border-[#4E653D]/25 rounded-xl text-sm font-semibold text-[#4E653D] shadow-sm">
                ...
                {{ $pendingCount }} {{ __('app.waiting_scan_qr') }}
            </div>
            <div class="inline-flex items-center gap-2 px-4 py-2.5 bg-[#4A2F24]/8 border border-[#4A2F24]/20 rounded-xl text-sm font-semibold text-[#4A2F24] shadow-sm">
                <span class="w-2.5 h-2.5 rounded-full bg-[#4A2F24] animate-pulse inline-block"></span>
                {{ $ongoingCount }} {{ __('app.currently_visiting') }}
            </div>
            <div class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 shadow-sm">
                {{ $pendingCount + $ongoingCount }} {{ __('app.total_active') }}
            </div>
        </div>
            ...
            <div class="px-4 sm:px-6 pt-4 pb-3 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="inline-flex items-center bg-gray-100 rounded-full p-1 text-xs font-medium">
                    <button type="button" wire:click="setTab('pending')"
                        ...
                        &#9201; {{ __('app.waiting_scan_tab') }}
                        @if($pendingCount > 0)
                            <span class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#354C2B] text-white text-[9px] flex items-center justify-center font-bold leading-none">
                    ...
                    <button type="button" wire:click="setTab('ongoing')"
                        class="relative px-4 py-1.5 rounded-full transition
                            {{ $activeTab === 'ongoing'
                                ? 'bg-[#4A2F24] text-[#CDDEA7] shadow-sm'
                                : 'text-gray-700 hover:bg-gray-200' }}">
                        &#128203; {{ __('app.visiting_tab') }}
                        @if($ongoingCount > 0)
                            <span class="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#2d1c15] text-[#CDDEA7] text-[9px] flex items-center justify-center font-bold leading-none">
            ...
            <div class="p-4 sm:p-6">
                    ...
                    @if($pendingEntries->isEmpty())
                        <div class="py-16 text-center text-gray-500 text-sm">
                            <div class="w-14 h-14 mx-auto mb-3 rounded-full bg-[#4E653D]/10 border border-[#4E653D]/20 flex items-center justify-center">
                                <x-heroicon-o-qr-code class="w-7 h-7 text-[#4E653D]"/>
                            </div>
                            <p class="font-semibold text-gray-700">{{ __('app.no_waiting_guests') }}</p>
                            <p class="text-xs text-gray-400 mt-1">{{ __('app.no_waiting_guests_sub') }}</p>
                        </div>
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            @foreach($pendingEntries as $e)
                                @php $avatarChar = strtoupper(substr($e->name ?? 'G', 0, 1)); @endphp
                                <div wire:key="pending-{{ $e->guestbook_id }}"
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center justify-between gap-2">
                                                ...
                                                <span class="shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#4E653D]/10 text-[#4E653D] border border-[#4E653D]/25">
                                                    &#9201; {{ __('app.pending') }}
                                                </span>
                                            </div>
                                            @if($e->instansi)
                                    ...
                                    {{-- QR info --}}
                                    @if($e->qr_token)
                                        <div class="flex items-center gap-2 text-[11px] text-[#4E653D] bg-[#4E653D]/5 rounded-lg px-2.5 py-1.5 border border-[#4E653D]/15">
                                    ...
                                    @else
                                        <div class="flex items-center gap-2 text-[11px] text-gray-400 bg-gray-50 rounded-lg px-2.5 py-1.5 border border-gray-100">
                                            <x-heroicon-o-envelope-open class="w-3.5 h-3.5 shrink-0"/>
                                            <span>{{ __('app.no_email_no_qr') }}</span>
                                        </div>
                                    @endif
                                    ...
                                    <div class="pt-2 border-t border-gray-100 flex items-center justify-end gap-1.5">
                                        ...
                                        @if($e->qr_token && $e->email)
                                            <button wire:click="resendQr({{ $e->guestbook_id }})"
                                                    ...
                                                    class="px-2.5 py-1.5 text-xs font-semibold rounded-lg text-[#4A2F24] bg-[#4A2F24]/8 border border-[#4A2F24]/20 hover:bg-[#4A2F24]/15 transition focus:outline-none">
                                                {{ __('app.resend_qr') }}
                                            </button>
                                        @endif
                ...
                @if($activeTab === 'ongoing')
                    @if($ongoingEntries->isEmpty())
                        <div class="py-16 text-center text-gray-500 text-sm">
                            <div class="w-14 h-14 mx-auto mb-3 rounded-full bg-[#4A2F24]/8 border border-[#4A2F24]/15 flex items-center justify-center">
                                <x-heroicon-o-user-group class="w-7 h-7 text-[#4A2F24]/60"/>
                            </div>
                            <p class="font-semibold text-gray-700">{{ __('app.no_visiting_guests') }}</p>
                            <p class="text-xs text-gray-400 mt-1">{{ __('app.no_visiting_guests_sub') }}</p>
                        </div>
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            @foreach($ongoingEntries as $e)
                                ...
                                <div wire:key="ongoing-{{ $e->guestbook_id }}"
                                     class="bg-white border border-[#4A2F24]/20 rounded-xl p-4 flex flex-col gap-3 hover:shadow-md hover:border-[#4A2F24]/35 transition">
                                    {{-- Header --}}
                                    <div class="flex items-start gap-3">
                                        <div class="{{ $icoAvatar }} bg-[#4A2F24]">{{ $avatarChar }}</div>
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center justify-between gap-2">
                                                <p class="font-semibold text-gray-900 truncate">{{ $e->name }}</p>
                                                <span class="shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#4A2F24]/10 text-[#4A2F24] border border-[#4A2F24]/20">
                                                    &#128203; {{ __('app.ongoing') }}
                                                </span>
                                            </div>
                                            @if($e->instansi)
                                    ...
                                    <div class="space-y-1 text-xs text-gray-600 bg-gray-50 rounded-lg p-2.5 border border-gray-100">
                                        ...
                                        <div class="flex gap-1.5">
                                            <span class="text-gray-400 shrink-0">{{ __('app.visitors_count') }}:</span>
                                            <span class="font-bold text-[#4A2F24]">{{ $e->visitor_count }}</span>
                                        </div>
                                        <div class="flex gap-1.5">
                                            <span class="text-gray-400 shrink-0">{{ __('app.officer_label') }}:</span>
                                            <span class="font-medium text-gray-700 truncate">{{ $e->petugas_penjaga }}</span>
                                    ...
                                    @if($scans->count())
                                        <div class="rounded-lg border border-[#4A2F24]/15 bg-[#4A2F24]/5 px-2.5 py-2 space-y-1">
                                            ...
                                            @if($e->visitor_count > 5)
                                                <p class="text-[10px] text-[#4A2F24]/50 text-center pt-0.5">
                                                    {{ str_replace(':count', $e->visitor_count - 5, __('app.plus_others')) }}
                                                </p>
                                            @endif
                                        </div>
                                    @endif
                                    ...
                                    <div class="pt-2 border-t border-gray-100 flex items-center justify-end gap-1.5">
                                        ...
                                        <button wire:click="checkOutNow({{ $e->guestbook_id }})"
                                                wire:confirm="{{ str_replace(':count', $e->visitor_count, __('app.checkout_group_confirm')) }}"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-[#4E653D] text-white hover:bg-[#354C2B] transition shadow-sm focus:outline-none">
                                            <x-heroicon-o-arrow-right-start-on-rectangle class="w-3.5 h-3.5"/>
                                            {{ __('app.checkout_btn') }}
                                        </button>
